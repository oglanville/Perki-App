...
/* ── Change #2: Tooltip with "Will not use" checkbox ── */
function PerkTooltip({perk, onToggle, onDismiss}) {
  return (
    <div onClick={e=>e.stopPropagation()} style={{padding:"12px 14px",borderRadius:12,background:"#1E293B",color:"#F1F5F9",fontSize:12,lineHeight:1.5,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 6px 20px rgba(0,0,0,0.18)",zIndex:10}}>
      ...
      <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:10,fontSize:11,color:"#94A3B8"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:12,fontWeight:600,color:"#fff"}}>
          <input type="checkbox" checked={!!perk.used} onChange={()=>onToggle(perk.perk_id)} disabled={!!perk.dismissed} style={{width:17,height:17,accentColor:T.accent,cursor:"pointer"}}/>
          {perk.used?"Marked as used":"Mark as used"}
        </label>
        <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:12,fontWeight:600,color:perk.dismissed?"#F87171":"#94A3B8"}}>
          <input type="checkbox" checked={!!perk.dismissed} onChange={()=>onDismiss(perk.perk_id)} style={{width:17,height:17,accentColor:"#F87171",cursor:"pointer"}}/>
...
/* ═══════════════ CURRENT MEMBERSHIPS ═══════════════ */
function MembershipsTab({perks,onToggle,onDismiss,activeMemberships}){const[brand,setBrand]=useState("");const[tier,setTier]=useState("");const[selected,setSelected]=useState(null);const activeProviders=[...new Set(activeMemberships.map(m=>m.provider))].sort();const activeTiers=brand?[...new Set(activeMemberships.filter(m=>m.provider===brand).map(m=>m.tier))]:[];const filtered=useMemo(()=>perks.filter(p=>(!brand||p.provider===brand)&&(!tier||p.tier===tier)).sort(alphaSort),[perks,brand,tier]);const countable=filtered.filter(p=>!p.dismissed);const used=countable.filter(p=>p.used).length;return(<div onClick={()=>setSelected(null)}><h1 style={{fontSize:22,fontWeight:800,color:T.textPrimary,margin:0,fontFamily:"'DM Sans',sans-serif"}}>Current Memberships</h1><TabDesc>Browse perks from your active memberships. Filter by provider or tier.</TabDesc><div style={{display:"flex",gap:8,marginBottom:14}}><Dropdown value={brand} onChange={v=>{setBrand(v);setTier("");}} options={activeProviders} placeholder="All Providers"/><Dropdown value={tier} onChange={setTier} options={activeTiers} placeholder="All Tiers" disabled={!brand}/></div>{filtered.length>0&&<p style={{fontSize:12,color:T.textSecondary,margin:"0 0 8px",fontFamily:"'DM Sans',sans-serif",fontWeight:600}}>Used {used} / Total {countable.length}</p>}<div style={{display:"flex",flexDirection:"column",gap:6}}>{filtered.map(p=><PerkTile key={p.perk_id} perk={p} onToggle={onToggle} onDismiss={onDismiss} selected={selected} onSelect={setSelected}/>)}</div>{filtered.length===0&&<p style={{textAlign:"center",color:T.muted,marginTop:40,fontSize:13,fontFamily:"'DM Sans',sans-serif"}}>Select a provider to see your perks</p>}</div>);}
...
function ProfileTab({perks,activeMemberships,onRemoveMembership,user,onLogout,onToggle,onDismiss}){const[expanded,setExpanded]=useState(null);const[profilePic,setProfilePic]=useState(null);const fileRef=useRef(null);const countable=perks.filter(p=>!p.dismissed);const used=countable.filter(p=>p.used).length;const sortedM=useMemo(()=>[...activeMemberships].sort((a,b)=>a.provider.localeCompare(b.provider)),[activeMemberships]);const handlePic=e=>{const f=e.target.files?.[0];if(f){const r=new FileReader();r.onload=ev=>setProfilePic(ev.target.result);r.readAsDataURL(f);}};
  return(<div><h1 style={{fontSize:22,fontWeight:800,color:T.textPrimary,margin:0,fontFamily:"'DM Sans',sans-serif"}}>Profile</h1><TabDesc>Manage your account, view stats, and control memberships.</TabDesc>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}><div style={{padding:14,borderRadius:12,textAlign:"center",background:"#F0FDF4",border:"1.5px solid #BBF7D0"}}><div style={{fontSize:26,fontWeight:800,color:T.success,fontFamily:"'DM Sans',sans-serif"}}>{countable.length-used}</div><div style={{fontSize:10,color:"#065F46",fontWeight:600,fontFamily:"'DM Sans',sans-serif"}}>Available</div></div><div style={{padding:14,borderRadius:12,textAlign:"center",background:"#EFF6FF",border:"1.5px solid #BFDBFE"}}><div style={{fontSize:26,fontWeight:800,color:T.accent,fontFamily:"'DM Sans',sans-serif"}}>{used}</div><div style={{fontSize:10,color:"#1E40AF",fontWeight:600,fontFamily:"'DM Sans',sans-serif"}}>Used</div></div></div>
