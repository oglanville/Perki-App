...
/* ═══════════════ HARD-CODED PERK BRANDS (fallback only — Supabase fields take priority) ═══════════════ */
const PERK_BRANDS = {
...
/* ═══════════════ FALLBACK TIER PRICES (used when Supabase tiers table is unavailable) ═══════════════ */
const FALLBACK_TIER_PRICES = {
  "OVO Energy|Beyond":{price_label:"Included with plan",sort_order:0},
  "Monzo|Extra":{price_label:"£4/mo",sort_order:0},"Monzo|Perks":{price_label:"£9/mo",sort_order:1},"Monzo|Max":{price_label:"£16/mo",sort_order:2},
  "Revolut|Standard":{price_label:"Free",sort_order:0},"Revolut|Plus":{price_label:"£3.99/mo",sort_order:1},"Revolut|Premium":{price_label:"£7.99/mo",sort_order:2},"Revolut|Metal":{price_label:"£14.99/mo",sort_order:3},"Revolut|Ultra":{price_label:"£45/mo",sort_order:4},
  "American Express|Gold":{price_label:"£250/yr",sort_order:0},"American Express|Platinum":{price_label:"£650/yr",sort_order:1},
};
 ...
 * Priority: fields on the Supabase perk row → hard-coded PERK_BRANDS fallback → generic default.
 * This fixes "??" for any perk_id that isn't in the hard-coded map.
 */
function getPerkBrand(perk){
  const hc=PERK_BRANDS[perk.perk_id]; // hard-coded entry (may be undefined)
  const provColor=(PROVIDERS[perk.provider]||{}).color||T.muted;
  return {
    emoji:     perk.emoji       || hc?.emoji    || "📦",
    initials:  perk.icon_initials || hc?.initials || perk.title?.slice(0,2).toUpperCase() || "??",
    color:     perk.icon_color   || hc?.color    || provColor,
    gradient:  perk.icon_gradient || hc?.gradient || `linear-gradient(135deg,${provColor}88,${provColor})`,
  };
}
...
function PotentialPerkTile({perk,selected,onSelect}){
  const pCfg=PROVIDERS[perk.provider]||{};
  const url=PERK_URLS[perk.perk_id]||perk.url;
...
/* ── PerkSquareTile (Home grid) — now reads emoji from perk data ── */
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:380,maxHeight:"80vh",overflowY:"auto",background:"#1E293B",borderRadius:16,padding:"20px 18px",color:"#CBD5E1",fontSize:12,lineHeight:1.6,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 20px 60px rgba(0,0,0,0.3)",position:"relative"}}>
        ...
        <div style={{fontWeight:700,color:"#fff",marginBottom:4}}>💳 Current Memberships</div><div style={{marginBottom:12}}>Filter your perks by provider and tier using the dropdowns.</div>
        ...
        <div style={{fontWeight:700,color:"#fff",marginBottom:4}}>✨ Potential Memberships</div><div style={{marginBottom:12}}>Browse all available memberships. Tiers you already have are marked "Already Active". Tap "+ Add Tier" to add a new one, or use + to request one we haven't listed.</div>
...
/* ═══════════════ PROFILE ═══════════════ */
function ProfileTab({perks,activeMemberships,onRemoveMembership,user,onLogout,onToggle,onDismiss,tierPrices}){const[expanded,setExpanded]=useState(null);const[selectedPerk,setSelectedPerk]=useState(null);const[profilePic,setProfilePic]=useState(null);const fileRef=useRef(null);const countable=perks.filter(p=>!p.dismissed);const used=countable.filter(p=>p.used).length;const willNotUseCount=perks.filter(p=>p.dismissed).length;const sortedM=useMemo(()=>[...activeMemberships].sort((a,b)=>{const c=a.provider.localeCompare(b.provider);if(c!==0)return c;return(tierPrices[`${a.provider}|${a.tier}`]?.sort_order??999)-(tierPrices[`${b.provider}|${b.tier}`]?.sort_order??999);}),[activeMemberships,tierPrices]);const handlePic=e=>{const f=e.target.files?.[0];if(f){const r=new FileReader();r.onload=ev=>setProfilePic(ev.target.result);r.readAsDataURL(f);}};
...
/* ═══════════════ POTENTIAL MEMBERSHIPS — shows ALL tiers, active ones marked ═══════════════ */
function PotentialTab({allPerks,activeMemberships,onAddMembership,userName,userId,tierPrices}){
  ...
  /* All brands and tiers (no longer filtering out active ones) */
  const availBrands=useMemo(()=>[...new Set(allPerks.map(p=>p.provider))].sort(),[allPerks]);
  const availTiers=useMemo(()=>{if(!brand)return[];return[...new Set(allPerks.filter(p=>p.provider===brand).map(p=>p.tier))];},[brand,allPerks]);

  /* Filtered perks — include ALL (active tiers stay visible) */
  const filtered=useMemo(()=>{let r=[...allPerks];if(brand)r=r.filter(p=>p.provider===brand);if(tier)r=r.filter(p=>p.tier===tier);if(search.trim()){const q=search.toLowerCase();r=r.filter(p=>p.title.toLowerCase().includes(q)||p.provider.toLowerCase().includes(q)||p.tier.toLowerCase().includes(q));}return r.sort(alphaSort);},[brand,tier,search,allPerks]);

  /* Group by provider|tier, sorted by sort_order (low→high price) */
  const tierGroups=useMemo(()=>{const g={};filtered.forEach(p=>{const k=`${p.provider}|${p.tier}`;if(!g[k])g[k]={provider:p.provider,membership:p.membership,tier:p.tier,perks:[]};g[k].perks.push(p);});return Object.values(g).sort((a,b)=>{const pc=a.provider.localeCompare(b.provider);if(pc!==0)return pc;return(tierPrices[`${a.provider}|${a.tier}`]?.sort_order??999)-(tierPrices[`${b.provider}|${b.tier}`]?.sort_order??999);});},[filtered,tierPrices]);
  ...
  return(<div onClick={()=>setSelected(null)}>
    ...
    <TabDesc>Browse all memberships. Tiers you already have are marked "Already Active".</TabDesc>
    ...
    {tierGroups.map(g=>{
      const isActive=activeSet.has(`${g.provider}|${g.tier}`);
      const tp=tierPrices[`${g.provider}|${g.tier}`];
      return(<div key={`${g.provider}|${g.tier}`} style={{marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6,padding:"9px 12px",borderRadius:12,background:isActive?"#F0FDF4":PROVIDERS[g.provider]?.bg||"#F1F5F9",border:`1.5px solid ${isActive?"#BBF7D0":(PROVIDERS[g.provider]?.color||T.border)+"22"}`}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <ProviderBadge provider={g.provider} size={28}/>
            <div>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:13,fontWeight:700,color:T.textPrimary,fontFamily:"'DM Sans',sans-serif"}}>{g.membership} — {g.tier}</span>
              ...
              <div style={{fontSize:10,color:T.textSecondary,fontFamily:"'DM Sans',sans-serif"}}>{g.perks.length} perks{tp?.price_label&&<span style={{marginLeft:6,fontWeight:700,color:PROVIDERS[g.provider]?.color||T.accent}}> · {tp.price_label}</span>}</div>
            </div>
          </div>
          {!isActive&&<button onClick={()=>onAddMembership(g.provider,g.membership,g.tier)} style={{padding:"6px 14px",borderRadius:10,border:"none",background:T.accent,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",boxShadow:"0 2px 6px rgba(30,144,255,0.3)"}}>+ Add Tier</button>}
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:5}}>{g.perks.map(p=><PotentialPerkTile key={p.perk_id} perk={p} selected={selected} onSelect={setSelected}/>)}</div>
      </div>);
    })}
    {tierGroups.length===0&&<p style={{textAlign:"center",color:T.muted,marginTop:30,fontSize:13,fontFamily:"'DM Sans',sans-serif"}}>{search?"No matching perks.":"No memberships found."}</p>}
  </div>);
}
...
export default function PerikiApp(){
  ...
  const[tierPrices,setTierPrices]=useState(FALLBACK_TIER_PRICES);
  ...
  /* Load tier prices from Supabase */
  useEffect(()=>{(async()=>{
    if(!SB_CONFIGURED)return; // keep fallback
    try{
      const{data,error}=await supabase.from("tiers").select("provider,tier,price_label,sort_order").order("sort_order");
      if(!error&&data?.length){
        const m={};data.forEach(r=>{m[`${r.provider}|${r.tier}`]={price_label:r.price_label,sort_order:r.sort_order??999};});
        setTierPrices(m);
      }
