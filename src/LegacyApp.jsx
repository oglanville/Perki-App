...
/* ═══════════════ THEME ═══════════════ */
const T={primary:"#2B2A6E",accent:"#3A388F",bg:"#F4F0E6",surface:"#FCFAF4",border:"#E4DDCB",textPrimary:"#23202A",textSecondary:"#6B6757",muted:"#8A8470",success:"#B07C1A",warning:"#B07C1A",danger:"#B4452F",shadow:"0 1px 2px rgba(43,42,40,.06)"};
const PROVIDERS={"OVO Energy":{color:"#00C86F",initials:"OV",bg:"#ECFDF5"},"Monzo":{color:"#FF5C5C",initials:"MZ",bg:"#FFF1F2"},"Revolut":{color:"#6C63FF",initials:"RV",bg:"#EEF2FF"},"American Express":{color:"#0077C8",initials:"AX",bg:"#F7ECD4"}};
...
const TABS=[{id:"home",label:"Home",icon:"🏠"},{id:"marketplace",label:"Marketplace",icon:"🛍️"},{id:"where",label:"Where",icon:"📍"},{id:"profile",label:"Profile",icon:"👤"}];
...
/* ── PerkSheet — single brand bottom-sheet detail surface (Phase 2) ── */
function PerkSheet({perk,mode="owned",onToggle,onDismiss,onClose}){
...
function MembershipsTab({perks,onToggle,onDismiss,activeMemberships,tierPrices}){
  ...
  return(<div onClick={()=>setSelected(null)}>
    <h1 style={{fontSize:22,fontWeight:800,color:T.textPrimary,margin:0,fontFamily:"'Outfit',sans-serif"}}>Current Memberships</h1>
    <TabDesc>Browse perks from your active memberships, grouped by provider and tier.</TabDesc>
    <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search perk name, provider, or tier..." style={{width:"100%",padding:"9px 14px",borderRadius:10,border:`1.5px solid ${T.border}`,background:T.surface,fontSize:13,fontFamily:"'Work Sans',sans-serif",color:T.textPrimary,marginBottom:10,boxSizing:"border-box",outline:"none"}}/>
    {filtered.length>0&&<p style={{fontSize:12,color:T.textSecondary,margin:"0 0 8px",fontFamily:"'Work Sans',sans-serif",fontWeight:600}}>Used {used} / Total {countable.length}</p>}
    ...
    {providerGroups.length===0&&<p style={{textAlign:"center",color:T.muted,marginTop:40,fontSize:13,fontFamily:"'Work Sans',sans-serif"}}>{search?"No matching perks.":"No active memberships yet."}</p>}
  </div>);
}
  ...
  return(
    <div style={{maxWidth:420,margin:"0 auto",minHeight:"100vh",background:T.bg,color:T.textPrimary,position:"relative",fontFamily:"'Work Sans',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,padding:"12px 16px 100px",overflowY:"auto"}}>
        ...
        {tab==="marketplace"&&<PotentialTab allPerks={allPerks} activeMemberships={activeMemberships} onAddMembership={addMembership} userName={user.name} userId={user.id} tierPrices={tierPrices}/>}
